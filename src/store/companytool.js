import { COMPANY_TOOL } from "@/graphql";
import { CompanyTool } from "@/models";
...
const actions = {
  ...
  async changeStatus(context, form) {
    ...
    const tool = new CompanyTool().deserialize(
      result.data.companyToolChangeStatus
    );
  ...
  async findById(context, filter) {
    ...
    try {
      ...
      const companyTool = new CompanyTool().deserialize(
        result.data.companyToolFindById
      );
...
const mutations = {
  ...
  SET_ITEM(state, value) {
    ...
    const index = state.all.findIndex(
      el =>
        //if module use toolId key as comparison.
        el[cacheKey] === value.id
    );
  ...
  SET_CURRENT_TOOL(state, { section, tool }) {
    AppStorage.set(
      "CURRENT_TOOL",
      Object.assign(AppStorage.get("CURRENT_TOOL", {}), {
        [section]: {
