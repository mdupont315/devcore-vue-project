...
import IdeaContent from "./ideacontent.model";
...
export default class IdeaModel extends BaseModel {
  ...
  get improvementsCount() {
    return this.stats && this.stats.improvements
      ? this.stats.improvements.total
      : 0;
  }
  ...
  get evaluationsCount() {
    return this.stats && this.stats.evaluations
      ? this.stats.evaluations.status.COMPLETED
      : 0;
  }
  ...
  deserialize(input) {
    ...
    if (input.ideaContent) {
      input.ideaContent = new IdeaContent().deserialize(input.ideaContent);
    }
