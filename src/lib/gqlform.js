import { showMessageFromResponse } from "./utils";
...
export default class GQLForm {
  ...
  get hasErrors() {
    return (
      this._errors && this._errors.details && this._errors.details.validation
    );
  }
  ...
  hasError(key) {
    const has =
      ...
      this._errors.details.validation[key].length > 0;
    return has;
  }
  ...
  getError(key) {
    if (
      ...
      if (message) {
        ...
        // // const locallabel= "serverValidation.fields." + ret.context.label.replace(".", "_");
        // // ret.context.label=window.vm.$t(locallabel)?window.vm.$t(locallabel, ret.context):ret.context.locallabel;

        // let messagekey = "serverValidation.messages." + ret.key + '_' + ret.type;
        // let translation = window.vm.$te(messagekey) ? window.vm.$t(messagekey, ret.context) : null;

        // if (!translation) {
        //     messagekey = "serverValidation.messages." + ret.type;
        //     translation = window.vm.$te(messagekey) ? window.vm.$t(messagekey, ret.context) : null;
        // }

        // if (!translation) {
        //     messagekey = "serverValidation.messages." + ret.type.replace(".", "_");
        //     translation = window.vm.$te(messagekey) ? window.vm.$t(messagekey, ret.context) : null;
        // }

        // ret.message = translation || ret.message;
        return ret;
      }
    }
    return null;
  ...
  clearError(key) {
    if (
      this.hasErrors &&
      this._errors.statusCode === 400 &&
      this._errors.details[key]
    ) {
