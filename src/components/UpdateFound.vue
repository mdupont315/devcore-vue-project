<template>
  ...
  >
    ...
    <div class="d-flex align-content-stretch">
      <b-button @click="refreshWindow" class="flex-grow-1" size="sm" variant="success">{{ $t('Refresh now!') }}</b-button>
      <b-button @click="dismiss" class="flex-grow-1 text-gray" size="sm" variant="link">{{ $t('Dismiss') }}</b-button>
    </div>
  </div>
</template>
<script>
export default {
    name:'sw-update-found',
    methods:{
        refreshWindow(){
